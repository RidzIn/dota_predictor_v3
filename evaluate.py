from prediction import get_onehot_prediction, get_winrates_prediction
import pandas as pd
import numpy as np
from copy import copy


def evaluate_tournament(tournament, prediction_method='NN'):
    """
    ...
    for row in tournament.itertuples():
        ...
        if prediction_method == 'NN':
            ...
            prediction_df = get_winrates_prediction(row.dire_heroes, row.radiant_heroes)
    ...
    print(f"Accuracy: {accuracy:.2%} | Right: {sum(is_correct)} | Wrong: {len(tournament) - sum(is_correct)}")
    print("-------------------------------")
    return df
